...
fn process() -> Result<()> {
   ...
   let bin_file_path = Path::new(&args.bin_file)
   .canonicalize()?;
   ...
   let bin_file_name = bin_file_path.file_name()
   .ok_or(NotAFileError)?;

   println!("Selected executable file:       {}", bin_file_path.display());
   ...
   let desk_file_path = local_apps_path.join(&desk_file_stem).with_extension(DESK_EXT);
   
   println!("Target .desktop file:           {}", desk_file_path.display());
   ...
   match args.icon {
      ...
      None => {
         ...
         icon_path = local_icons_path.join(&desk_file_stem).with_extension(ICON_EXT);
